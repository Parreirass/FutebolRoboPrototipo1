...
def click_event1(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        tolerance = 5
        lower_yellow = np.array([max(hsv_color[0] - tolerance, 0), 100, 100])
        upper_yellow = np.array([min(hsv_color[0] + tolerance, 179), 255, 255])
...
def select_yellow_color():
    ...
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)  # Largura em pixels
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)  # Altura em pixels
...
def click_event2(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        # Define os limites com uma tolerância ao redor da cor clicada
        tolerance = 20
        lower_pink = np.array([max(hsv_color[0] - tolerance, 0), 100, 100])
        upper_pink = np.array([min(hsv_color[0] + tolerance, 179), 255, 255])
...
def click_event3(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        # Define os limites com uma tolerância ao redor da cor clicada
        tolerance = 20
        lower_green = np.array([max(hsv_color[0] - tolerance, 0), 100, 100])
        upper_green = np.array([min(hsv_color[0] + tolerance, 179), 255, 255])
...
def click_event4(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        # Define os limites com uma tolerância ao redor da cor clicada
        tolerance = 20
        lower_blue = np.array([max(hsv_color[0] - tolerance, 0), 100, 100])
        upper_blue = np.array([min(hsv_color[0] + tolerance, 179), 255, 255])
...
def click_event(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        # Define os limites com uma tolerância ao redor da cor clicada
        tolerance = 20
        lower_orange = np.array([max(hsv_color[0] - tolerance, 0), 100, 100])
        upper_orange = np.array([min(hsv_color[0] + tolerance, 179), 255, 255])
...
def detect_colors_with_contours():
    ...
    display_interval = 0.02  # Intervalo de exibição em segundos
    last_display_time = 0
    while True:
        ret, frame = cap.read()
...
def main():
    if select_orange_color() and select_blue_color() and select_green_color() and select_pink_color() and select_yellow_color():
