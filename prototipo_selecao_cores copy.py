...
def coef_calc(uv_points, xy_points):
    #Montando a matriz A:
    A = np.block([np.ones((len(uv_points),1)), np.array(uv_points)])
...
def uv_to_xy(uv, consts):
    coefs = np.vstack([consts[0], consts[1], np.zeros(len(consts[0]))])
    imag = np.hstack((np.ones(1),np.array(uv)))[:, np.newaxis]
    out = (coefs@imag).T
    return out[0][:2].tolist()
...
def select_points():
    ...
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)  # Largura em pixels
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)  # Altura em pixels
...
def detect_colors_with_contours():
    ...
    display_interval = 5  # Intervalo de exibição em segundos
    last_display_time = 0
    while True:
        ret, frame = cap.read()
        print('\n\n---------------------------------------------------------------------------------------------------------\n\n')
                        ...
                        if M["m00"] != 0:
                            ...
                            for secondary1_contour in secondary1_contours:
                                ...
                                if secondary1_moment["m00"] != 0:
                                    ...
                                    print(f'A-R: ({rx}, {ry}, {np.degrees(np.arctan2((secondary1_cY-cY), (secondary1_cX-cX)))})')
                            ...
                            for secondary2_contour in secondary2_contours:
                                ...
                                if secondary2_moment["m00"] != 0:
                                    ...
                                    rx,ry = uv_to_xy((cX,cY), cte)
                                    print(f'A-A: ({rx}, {ry}, {np.degrees(np.arctan2((secondary2_cY-cY), (secondary2_cX-cX)))})')
                        ...
                        if M["m00"] != 0:
                            ...
                            for secondary1_contour in secondary1_contours:
                                ...
                                if secondary1_moment["m00"] != 0:
                                    ...
                                    cv2.circle(frame, (secondary1_cX, secondary1_cY), 3, (0, 0, 255), -1)
                                    rx,ry = uv_to_xy((cX,cY), cte)
                                    print(f'V-R: ({rx}, {ry}, {np.degrees(np.arctan2((secondary1_cY-cY), (secondary1_cX-cX)))})')
                            ...
                            for secondary2_contour in secondary2_contours:
                                ...
                                if secondary2_moment["m00"] != 0:
                                    ...
                                    cv2.circle(frame, (secondary2_cX, secondary2_cY), 3, (0, 0, 255), -1)
                                    rx,ry = uv_to_xy((cX,cY), cte)
                                    print(f'V-A: ({rx}, {ry}, {np.degrees(np.arctan2((secondary2_cY-cY), (secondary2_cX-cX)))})')
